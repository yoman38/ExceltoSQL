...
Sub GenerateSQL()
    ...
    ' Generate SQL
    sqlCreate = GenerateCreateTable(rngHeaders, rngData, fso.GetBaseName(inputFilePath))
...
End Function
Function GenerateCreateTable(headers As Range, data As Range, tableName As String) As String
    Dim i As Long
    Dim j As Long
    Dim sqlCreate As String
...
For i = 1 To headers.Columns.Count
        ...
        For j = 1 To numRows
            guessedType = GuessDataType(data.Cells(j, i).value)
            ' If any non-string type is found, we can stop checking
            If guessedType <> "NVARCHAR(10)" Then Exit For
        ...
        ' If the datatype is string, we use the maximum string length found
        If guessedType = "NVARCHAR(10)" Then
...
Function GuessDataType(value As Variant) As String
    ...
    ' Check for binary first, before checking if it's numeric
    If TypeName(value) = "String" Then
        ' Check for binary
        regex.Pattern = "^[01]+$"
        If regex.Test(value) Then
            GuessDataType = "BINARY"
            Exit Function
        End If
    End If
